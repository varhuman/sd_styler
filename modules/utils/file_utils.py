...
def get_dirs(path):
    dirs = []
    if not os.path.exists(path):
        print(f"get_dirs: the path {path} is not exist")
        return dirs
...
def write_json_file(path:str, content):
    ...
    if not path.endswith(".json"):
        print(f"write_json_file: the file {path} is not a json file")
        return False
...
def read_json_file(path:str):
    #先检查是否是json文件
    if not path.endswith(".json"):
        print(f"read_json_file: the file {path} is not a json file")
        return False
...
def create_folder(path:str, folder):
    if not check_folder_is_exist(path, folder):
        ...
        print(f"create_folder: the folder {folder} is already exist")
        return False