...
class Txt2ImgModel(BaseModel):
    ...
    def get_attribute_value(self, attribute_name):
        if attribute_name == "sd_model_checkpoint":
            ...
            model_name = image_utils.get_model_name_from_title(title)
    ...
    def set_override_settings(self, model):
        ...
        title = image_utils.get_title_from_model_name(model)
...
# 上面的所有参数写出来
    ...
    def custom_to_dict(self):
        res_dict = self.dict()
        init_image = image_utils.image_path_to_base64(self.init_images[0])
        res_dict["init_images"][0] = init_image
        res_dict["mask"] = image_utils.image_path_to_base64(self.mask)
        controlnet_args:ControlNet_Model = self.get_controlnet_params()
        if controlnet_args:
            res_dict["alwayson_scripts"]["ControlNet"]["args"]["image"] = image_utils.image_path_to_base64(controlnet_args["image"])
            res_dict["alwayson_scripts"]["ControlNet"]["args"]["mask"] = image_utils.image_path_to_base64(controlnet_args["mask"])
        return res_dict
...
def parse_string_to_img2img_model(s: str) -> Img2ImgModel:
    ...
    *lines, lastline = s.strip().split("\n")
    ...
    checkpoint_model = image_utils.get_model_name_from_hash(model_hash) 
